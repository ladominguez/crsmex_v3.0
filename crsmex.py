...
def get_correlation_coefficient(*args):
    ...
    year_1  = sac1.stats.starttime.year
    month_1 = sac1.stats.starttime.month
    day_1   = sac1.stats.starttime.day

    
    year_2  = sac2.stats.starttime.year
    month_2 = sac2.stats.starttime.month
    day_2   = sac2.stats.starttime.day
    ...
    if Win < 0: # Select all, I did this to check Taka's Java code. If Win is negative takes the whole record
        Win   = min(sac1.stats.delta*(sac1.stats.npts -1), sac2.stats.delta*(sac2.stats.npts -1))
        Ntrim = min(sac1.stats.npts, sac2.stats.npts)
        T1    = T1[:Ntrim]
        T2    = T2[:Ntrim]
        S1    = S1[:Ntrim]
        S2    = S2[:Ntrim]
...
#                           Luis A. Dominguez (2014)
    ...
    if sac_file.stats.sac.a  != -12345.0:
        start_time = sac_file.stats.sac.a
    elif sac_file.stats.sac.t5 != -12345.0:
        start_time = sac_file.stats.sac.t5
    else:
        sys.exit('Not enough header information - a and t5 missing.')

    time  = np.linspace(sac_file.stats.sac.b, sac_file.stats.sac.e, sac_file.stats.npts )
    data  = sac_file.data    
